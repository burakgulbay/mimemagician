...
import (
	...
	"github.com/burakgulbay/mimemagician"
)
...
var (
	...
	preferGlob      bool
	preference      int
	mimeType        mimemagician.MediaType
	input           *os.File
	err             error
	limit           int
	files           []string
...
func setup() {
	...
	if preferGlob {
		preference = mimemagician.Glob
	}
	if preferMagic {
		preference = mimemagician.Magic
	}
	files = flag.Args()
	if files[0] == "-" {
		filenameOnly = false
...
func identify(filename string) {
	switch {
	case contentOnly:
		mimeType, err = mimemagician.MatchReader(input, "", limit)
	case filenameOnly:
		mimeType = mimemagician.MatchGlob(filepath.Base(filename))
	case treeMagic:
		mimeType, err = mimemagician.MatchTreeMagic(filename)
	case xmlNamespace:
		mimeType = mimemagician.MatchXMLReader(input, limit)
	default:
		mimeType, err = mimemagician.MatchFile(input, limit, preference)
	}
	if !printError(err) {
		if prependFilename {
			if !humanReadable {
